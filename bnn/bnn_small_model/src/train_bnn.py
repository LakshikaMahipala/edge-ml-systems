...
def main():
    ...
    ap.add_argument("--out", type=str, default="results/bnn_metrics.json")
    args = ap.parse_args()
    ...
    for e in range(args.epochs):
        ...
        print(f"[BNN] epoch {e+1:02d}: loss={loss:.4f} val_acc={acc:.4f}")

    model.eval()
    t = wall_ms(lambda: model(Xva[:1]))
    metrics = {"mode": "bnn", "val_acc": float(eval_model(model, Xva, yva)), "p50_ms_est": float(t)}
